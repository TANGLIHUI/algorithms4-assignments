...
public class BoggleGame extends JFrame {
    ...
    private static final int SECONDS_PER_MINUTE = 60; // number of seconds in
                                                      // one minute
    private static final int FOUND_WORDS_DISPLAY_COUNT = 17; // how many rows to
                                                             // display for the
                                                             // two side columns
    private static final int ALL_WORDS_DISPLAY_COUNT = 7; // how many rows to
                                                          // display for the
                                                          // middle all-words
                                                          // list
    ...
    // if making adjustments to levels, endGame (~line 400) contains hard-coded
    // elements
    // menu items will be adjusted automatically
    private static final int NUMBER_OF_LEVELS = 5;
    private static final String[] LEVEL_DESCRIPTION = { "Nursery", "Shakespeare", "Algorithms 4/e", "Hard",
            "Impossible" };
    ...
    private LinkedHashSet<String> foundWords; // to keep words in same order as
                                              // entered
    ...
    // dictionaries
    // (words that appear in Shakespeare, nursery rhymes, common words, and
    // Algorithms 4/e)
    ...
    public BoggleGame(int rows, int cols) {
        ...
        entryField.setMaximumSize(
                new Dimension(entryField.getPreferredSize().width, entryField.getPreferredSize().height));
        ...
        controlLayout.setHorizontalGroup(controlLayout.createSequentialGroup()
                .addGroup(controlLayout.createParallelGroup(GroupLayout.Alignment.CENTER).addComponent(timerPanel)
                        .addComponent(entryField).addComponent(foundWordsScrollPane).addComponent(scoreLabelPanel)));
        controlLayout.setVerticalGroup(controlLayout.createSequentialGroup()
                .addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addComponent(timerPanel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                .addComponent(entryField, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
        ...
        // creates the board and the list that will show all the available words
        ...
        gameLayout.setHorizontalGroup(
                gameLayout.createSequentialGroup().addGroup(gameLayout.createParallelGroup(GroupLayout.Alignment.CENTER)
                        .addComponent(bp).addComponent(validWordsScrollPane).addComponent(possiblePointsPanel)));
        ...
        buttonsLayout.setHorizontalGroup(buttonsLayout.createSequentialGroup()
                .addGroup(buttonsLayout.createParallelGroup(GroupLayout.Alignment.CENTER).addComponent(spacingPanel)
                        .addComponent(opponentLabelPanel).addComponent(opponentWordsScrollPane)
        ...
        buttonsLayout.setVerticalGroup(buttonsLayout.createSequentialGroup()
                .addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                .addComponent(spacingPanel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                ...
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                .addComponent(opponentWordsScrollPane, GroupLayout.PREFERRED_SIZE, GroupLayout.PREFERRED_SIZE,
                        GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                ...
                // .addComponent(winnerLabel, GroupLayout.PREFERRED_SIZE,
                // GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE)
                .addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
        ...
        layout.setHorizontalGroup(layout.createSequentialGroup()
                ...
                .addComponent(controlPanel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                ...
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.DEFAULT_SIZE)
                .addComponent(opponentPanel, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE,
                        GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(LayoutStyle.ComponentPlacement.UNRELATED, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE));
        layout.setVerticalGroup(
                layout.createSequentialGroup().addGroup(layout.createParallelGroup(GroupLayout.Alignment.CENTER, false)
                        .addComponent(controlPanel).addComponent(gamePanel).addComponent(opponentPanel)));

        String dir = "E:/WorkSpace1/algorithm/src/boggle/";
        // all words in shakespeare
        In in1 = new In(new File(dir + "dictionary-shakespeare.txt"));
        shakespeareDictionary = new SET<String>();
        for (String s : in1.readAllStrings())
            shakespeareDictionary.add(s);

        // all words in shakespeare
        In in2 = new In(new File(dir + "dictionary-nursery.txt"));
        nurseryDictionary = new SET<String>();
        for (String s : in2.readAllStrings())
            nurseryDictionary.add(s);

        // about 20K common words
        In in3 = new In(new File(dir + "dictionary-common.txt"));
        commonDictionary = new SET<String>();
        for (String s : in3.readAllStrings())
            commonDictionary.add(s);

        // all words in Algorithms 4/e
        In in4 = new In(new File(dir + "dictionary-algs4.txt"));
        algs4Dictionary = new SET<String>();
        for (String s : in4.readAllStrings())
            algs4Dictionary.add(s);

        // dictionary
        In in = new In(new File(dir + "dictionary-yawl.txt"));
    ...
    private class BoardPanel extends JPanel {
                        ...
                        if (inGame) {
                            ...
                            for (int j = 0; j < path.length; j++) {
                                ...
                                // if the cube clicked is in the path
                                    ...
                                    // check if it is the last cube or the last
                                    // one in the current path
                                ...
                                // check for adjacency to the last cube in the
                                // path
                                else if (path[j] == -1) {
                                    // row above
        ...
        private void dfs(String s, int curChar, int pathIndex, int i, int j) {
            ...
            if (curChar != 0 && s.charAt(curChar - 1) == 'Q' && s.charAt(curChar) != 'U')
                return;
            // increment character count if it is a 'U' after a 'Q' and keep
            // searching
            if (curChar != 0 && s.charAt(curChar - 1) == 'Q' && s.charAt(curChar) == 'U')
                curChar += 1;
            if (curChar >= s.length()) {
    ...
    private void makeMenuBar() {
        ...
        endGameMenuItem.setAccelerator(
        ...
        for (int i = 0; i < NUMBER_OF_LEVELS; i++) {
            difficultySelection[i] = new JRadioButtonMenuItem(LEVEL_DESCRIPTION[i % LEVEL_DESCRIPTION.length]); // mod
                                                                                                                // as
                                                                                                                // a
                                                                                                                // check
                                                                                                                // against
